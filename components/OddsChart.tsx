...
export function OddsChart({
    ...
    (async () => {
      ...
      await Promise.all(
        players.map(async (p) => {
          let rows: RawRow[] = [];
          if (gameIds.length === 0) {
            const res = await fetch(`/api/players/${p.player_id}/odds?market_key=${marketKey}&t=${Date.now()}`, { cache: "no-store" });
            const json = await res.json();
            if (json.ok) rows = json.data as RawRow[];
          } else {
            const parts = await Promise.all(
              gameIds.map(async (gid) => {
                const res = await fetch(`/api/players/${p.player_id}/odds?market_key=${marketKey}&game_id=${gid}&t=${Date.now()}`, { cache: "no-store" });
                const json = await res.json();
  ...
  // interactivity / layout (same goodies you already had)
  ...
  useEffect(() => { if (tsMin !== undefined && tsMax !== undefined) setXDomain([tsMin, tsMax]); }, [tsMin, tsMax, players.map((p) => p.player_id).join(",")]);
  ...
  function zoom(factor: number) {
    ...
    setXDomain([Math.max(tsMin ?? a, center - half), Math.min(tsMax ?? b, center + half)]);
  }
      ...
      {!hasData ? (
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            {/* Lines: two per player. Hover locks tooltip to the exact series */}
