...
export function OddsChart({
  ...
  // fetch + merge (supports multiple games)
  useEffect(() => {
    let aborted = false;
    (async () => {
  ...
  const [height, setHeight] = useState(680); // larger by default
  const [showFD, setShowFD] = useState(true);
  const [showMGM, setShowMGM] = useState(true);
  const [showDots, setShowDots] = useState(true);
  const [smooth, setSmooth] = useState(false);

  // lines (with logo-driven hover behavior)
...
// helpers/handlers used above (kept here for clarity)
function useHandlers(xDomain: [number, number] | undefined) {
  return;
}
