      ...
      <div className="max-h-96 overflow-auto divide-y">
        {filtered.map((g: any) => {
          ...
          return (
            <label
              key={g.game_id}
              className={`flex items-center justify-between gap-2 px-2 py-2 text-sm cursor-pointer ${checked ? "bg-blue-50" : ""}`}
              onClick={() => toggle(g.game_id)}
            >
              <div className="flex flex-col">
                <span className="font-medium">{g.away_team} @ {g.home_team}</span>
                <span className="text-xs text-gray-500">{new Date(g.commence_time).toLocaleString()}</span>
              </div>
              <input type="checkbox" className="h-4 w-4" onChange={() => {}} checked={checked} />
            </label>
          );
        })}
        {!filtered.length && <div className="text-sm text-gray-500 p-4">No games match your search.</div>}
      </div>
    </div>
  );
}
